...
class ApplicationCustomEditForm(forms.ModelForm):
    ...
    subjects = forms.ModelMultipleChoiceField(label='Предметы к обучению',
                                              queryset=Subject.objects.filter(is_deleted=False), required=False,
    ...
    phone = PhoneNumberField(region='KZ', label='Телефон', required=False,
    ...
    lesson_time = forms.ChoiceField(label='Желательное время', choices=LESSONS_TIME, required=False,
                                    widget=forms.Select(attrs={'class': 'form-control'}))
    birth_date = forms.DateField(widget=DateInput(format='%Y-%m-%d', attrs={'class': 'form-control'}), required=False,
                                 label='Дата рождения')
    ...
    class Meta:
        ...
        widgets = {
            ...
            'parents_inn': forms.NumberInput(attrs={'class': 'form-control'}),
